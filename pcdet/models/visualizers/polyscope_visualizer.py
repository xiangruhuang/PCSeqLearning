...
class PolyScopeVisualizer(nn.Module):
    def __init__(self, model_cfg, **kwargs):
        ...
        if self.enabled:
            ...
            self.box_sequence_from_corners_vis = model_cfg.get("BOX_SEQUENCE_FROM_CORNERS", None)
        ...
        """
            points (dictionary):
                ...
                additional attributes:
                    points.scalars (dictionary): (key, Attribute) pairs
                    points.colors (dictionary): (key, Attribute) pairs
                    points.vectors (dictionary): (key, Attribute) pairs
        """
        ps_p = ps.register_point_cloud(points.name, to_numpy_cpu(points.xyz), radius=points.radius, **points.kwargs)
        for scalar_name, scalar in points.scalars.items():
            ps_p.add_scalar_quantity(scalar_name, to_numpy_cpu(scalar.values), **scalar.kwargs)
        for color_name, color in points.colors.items():
            ps_p.add_color_quantity(color_name, to_numpy_cpu(color), **color.kwargs)
        for vector_name, vector in points.vectors.items():
            ps_p.add_vector_quantity(vector_name, to_numpy_cpu(vector), **vector.kwargs)
    ...
    def register_boxes(self, boxes):
        ...
        for scalar_name, scalar in boxes.scalars.items():
            ps_box.add_scalar_quantity(scalar_name, to_numpy_cpu(scalar.value), **scalar.kwargs)
        for color_name, color in boxes.colors.items():
            ps_box.add_color_quantity(color_name, to_numpy_cpu(color.value), **color.kwargs)
        for vector_name, vector in boxes.vectors.items():
            ps_box.add_vector_quantity(vector_name, to_numpy_cpu(vector.value), **vector.kwargs)
    ...
    def forward(self, batch_dict):
        ...
        for i in range(batch_dict.get('batch_size', 1)):
            ...
            if self.box_sequence_from_corners_vis is not None:
                for box_key, vis_cfg_this in self.box_sequence_from_corners_vis.items():
                    if box_key not in batch_dict:
                        continue
                    vis_cfg = {}; vis_cfg.update(vis_cfg_this)
                    corners = to_numpy_cpu(batch_dict[box_key]).reshape(-1, 8, 3)
                    mask = np.linalg.norm(corners, axis=-1, ord=2).sum(axis=-1) > 1e-1
                    #mask = (boxes[:, 3:6] ** 2).sum(axis=-1) > 1e-1
                    self.boxes(box_key, corners[mask], batch_dict, np.ones(corners.shape[0], dtype=bool), mask, None, **vis_cfg)
    ...
    def pointcloud(self, name, pointcloud, data_dict, batch_mask, color=None, radius=None, **kwargs):
        ...
        if pointcloud.shape[0] > 2000000:
            subsample = True
            from pcdet.models.model_utils.grid_sampling import GridSampling3D
            from torch_scatter import scatter
            grid_sampler = GridSampling3D(grid_size=[0.08, 0.08, 0.08]).cuda()
            pointcloud_torch = torch.from_numpy(pointcloud).cuda()
            pointcloud_torch = torch.cat([torch.zeros_like(pointcloud_torch[:, 0:1]), pointcloud_torch], dim=-1)
            _, inv = grid_sampler(pointcloud_torch, return_inverse=True)
            random_idx = scatter(torch.arange(pointcloud_torch.shape[0]).cuda(), inv, dim_size=inv.max().long()+1, dim=0,
                                 reduce='max').detach().cpu().numpy()
            pointcloud = pointcloud[random_idx]
        else:
            subsample = False
        ...
        if scalars:
            for scalar_name, scalar_cfg in scalars.items():
                ...
                try:
                    scalar = to_numpy_cpu(data_dict[scalar_name][batch_mask])
                    if subsample:
                        scalar = scalar.reshape(-1)[random_idx]
        ...
        if class_labels:
            for label_name, label_cfg in class_labels.items():
                ...
                if label.shape[0] == 0:
                    continue
                if subsample:
                    label = label.reshape(-1)[random_idx]
                label_cfg_this = {}
                for key, val in label_cfg.items():
                    if (key == 'values') and isinstance(val, str):
                        label_cfg_this[key] = self.color(val)[label]
        ...
        if class_labels is not None:
            for key, cfg in class_labels.items():
                ...
                for _key, val in cfg.items():
                    if (_key == 'values') and isinstance(val, str):

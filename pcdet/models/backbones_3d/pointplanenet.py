...
from pcdet.models.model_utils import graph_utils, sampler_utils
from .post_processors import build_post_processor
from pcdet.models.blocks import PointPlaneNetDownBlock, PointPlaneNetUpBlock, SelfAttentionBlock, PointPlaneNetFlatBlock
...
class PointPlaneNet(nn.Module):
    def __init__(self, runtime_cfg, model_cfg, **kwargs):
        ...
        plane_samplers = model_cfg.get("PLANE_SAMPLERS", None)
        ...
        channel_stack = []
        self.plane_samplers = nn.ModuleList()
        for i, sa_channel in enumerate(self.sa_channels):
            ...
            plane_sampler_cfg = common_utils.indexing_list_elements(plane_samplers, i)
            plane_sampler = sampler_utils.build_sampler(plane_sampler_cfg, runtime_cfg)
            self.plane_samplers.append(plane_sampler)
    ...
    def forward(self, batch_dict):
        ...
        for i, down_module in enumerate(self.down_modules):
            key = f'pointplanenet_down{len(self.sa_channels)-i}'
            #batch_dict[f'{key}_ref'] = point_bxyz
            points, planes, runtime_dict = down_module(points, planes, runtime_dict)
